...
if fn_is_realtime(Payment_Method)=1 and (fn_is_third_party_gateway(Real_Time_Processor)=1 or fn_is_paypal_express(Payment_Method,Real_Time_Processor) or fn_is_paypal(Payment_Method,Real_Time_Processor) or fn_is_checksbynet(Payment_Method,Real_Time_Processor)) then
   ...
   elseif fn_is_checksbynet(Payment_Method,Real_Time_Processor)=1 then
		%><!--#include file="include/checksbynet/checksbynetdisplay.asp"--><%
		call create_form_post_checksNet
...
sPayments="Visa,Mastercard,American Express,Discover,Diners Club,eCheck,Paypal,Debit Card,Checks by Net,Solo,Switch,Maestro"
if Is_In_Collection(sPayments,Payment_Method,",") then
    if fn_is_paypal(Payment_Method,Real_Time_Processor)=1 then
		fn_print_debug "in is paypal"
		call create_form_content_paypal
    elseif fn_is_checksbynet(Payment_Method,Real_Time_Processor)=1 then
		 call create_form_content_checksNet
	elseif Real_Time_Processor = 8 then
...
elseif (Payment_Method = "Visa" or Payment_Method = "Mastercard" or Payment_Method = "Discover" or Payment_Method = "American Express" or Payment_Method = "Diners club" or Payment_Method="eCheck" or Payment_Method="Debit Card" or Payment_Method="Checks By Net") then
