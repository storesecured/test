...
set oFolder_base0 = oFSO.GetFolder(arrBaseFolder(0)) 'base folder (Physical)
if arrBaseFolder(1)<>"" then set oFolder_base1 = oFSO.GetFolder(arrBaseFolder(1)) 'base folder (Physical)
if arrBaseFolder(2)<>"" then set oFolder_base2 = oFSO.GetFolder(arrBaseFolder(2)) 'base folder (Physical)
if arrBaseFolder(3)<>"" then set oFolder_base3 = oFSO.GetFolder(arrBaseFolder(3)) 'base folder (Physical)
...
function writeFileSelections()
	...
	for each oFile in oFiles
		...
		elseif ffilter="image" then
			...
			if LCase(sExt)<>"asp" then bDisplay=true
		end if
		'~~~~~~~~~~~~~~~~~~
		...
		if bDisplay then
			...
			'response.Write oFile.path & "  -  " & oFSO.GetFolder(sBaseFolder) & "<br>"
			if InStr(1,oFile.path,oFSO.GetFolder(arrBaseFolder(0))) <> 0 then
				sBaseFolder_PhysicalPath = replace(oFolder_base0.path,arrBaseFolder(0),"") 'folder terakhir tdk diikuti dgn / shg sFile_VirtualPath_UnderBaseFolder diawali dgn /
				sBaseFolder = arrBaseFolder(0)
			end if
			if arrBaseFolder(1)<>"" then'NEW 2.4
				if InStr(1,oFile.path,oFSO.GetFolder(arrBaseFolder(1))) <> 0 then
			...
			if arrBaseFolder(2)<>"" then'NEW 2.4
				if InStr(1,oFile.path,oFSO.GetFolder(arrBaseFolder(2))) <> 0 then
			...
			if arrBaseFolder(3)<>"" then'NEW 2.4
				if InStr(1,oFile.path,oFSO.GetFolder(arrBaseFolder(3))) <> 0 then
			...
			sFile_VirtualPath = CStr(sFile_VirtualPath_UnderBaseFolder)
			...
			Response.Write "<tr style='background:" & sColorResult & "'>" & VbCrLf & _
				...
				"<td valign=top align=right nowrap>" & FormatNumber(oFile.size/1000,1) & " kb&nbsp;</td>" & VbCrLf
...
function selectFile(index)
	...
	sFile_RelativePath = "<%= Site_Name %>" + sFile_RelativePath;
...
<td valign=top style="background:url('bg.gif') no-repeat right bottom;padding-top:5px;padding-left:5px;padding-right:5px;padding-bottom:0px;">
		...
		<td>
				...
				<td nowrap>
					<span onclick="newFolder()" style="cursor:pointer;"><u><span name="txtLang" id="txtLang" <%=sFolderAdmin%>></span></u></span>&nbsp;
					<span onclick="deleteFolder()" style="cursor:pointer;"><u><span name="txtLang" id="txtLang" <%=sFolderAdmin%>></span></u></span>
				</td>
     ...
     <!--ffilter-->
					...
					<span name="txtLang" id="txtLang"></span> <input type="hidden" id="File1" name="File1" class="inpTxt">&nbsp;
					<input name="btnUpload" id="btnUpload" type="hidden" value="upload" onclick="upload()" class="inpBtn" onmouseover="this.className='inpBtnOver';" onmouseout="this.className='inpBtnOut'">
					</form>
				</div>
		</td>
		</tr>
