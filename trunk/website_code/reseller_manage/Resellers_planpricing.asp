...
function fnsave()
{
	...
	else if (isWhitespace(document.frmplan.text12.value)== true)
		{
	...
	else if (isWhitespace(document.frmplan.text13.value)== true)
		{
	...
	if (isWhitespace(document.frmplan.text21.value)== true)
	...
	else if(isWhitespace(document.frmplan.text22.value)== true)
		{
			ErrMsg = ErrMsg + "Price for Bronze for range <50-100> is mandatory. \n" ;		
		}	
	...
	else if (isWhitespace(document.frmplan.text23.value)== true)
		{
			ErrMsg = ErrMsg + "Price for Bronze for range <100 & above> is mandatory. \n" ;		
		}	
		...
		else if (isWhitespace(document.frmplan.text32.value)== true)
		{
			ErrMsg = ErrMsg + "Price for Bronze for range <50-100> is mandatory. \n" ;		
		}	
		...
		else if (isWhitespace(document.frmplan.text33.value)== true)
		{
			ErrMsg = ErrMsg + "Price for Bronze for range <100 & above> is mandatory. \n" ;		
		}	
		...
		else if (isWhitespace(document.frmplan.text42.value)== true)
		{
			ErrMsg = ErrMsg + "Price for Bronze for range <50-100> is mandatory. \n" ;		
		}
		...
		else if (isWhitespace(document.frmplan.text43.value)== true)
		{
			ErrMsg = ErrMsg + "Price for Bronze for range <100 & above> is mandatory. \n" ;		
		}	
...
set strrs=conn.execute(str1)
	...
	if not strrs.eof then
		while not strrs.eof
			...
			planrate1(j)=trim(strrs("fld_rate"))
...
<% if intflag="1" then%>
<script language=javascript>
	alert("Easystorecreators plan prizing rates have been added.")
...
<% if intflag="2" then%>
<script language=javascript>
	alert("Easystorecreators plan prizing rates have been updated.")
	document.location.href = "resellers_planpricing.asp"
</script>
<% end if%>
