...
<BR><BR>Please completely fill out the form below to get started with your <%=Name %> site.
			...
			<form method="POST" target="_top" name=signup action="http://<%=manage_server %>/new_Store_Action.asp" onSubmit="javascript:__utmLinkPost(this)">
			...
			<tr>
					...
					<input type="text" name="Site_name" size="25" maxlength=63 onKeyPress="return goodchars(event,'0123456789abcdefghijklmnopqrstuvwxyz-')">.<%=root %>
					<input type="hidden" name="Site_name_C" value="Re|String|0|63||.,http,www,\,/,?,:,*,&,@,_|Web Address">
               <input type="hidden" name="site_host" value="<%=root %>"></font></td>
				</tr>
                            <tr>
...
You can review the most current version of the TOS at any time at: http://<%=sDefaultUrl %>/tos.asp
...
Registration Data and certain other information about you is subject to our Privacy Policy. For more information, please see our full privacy policy at http://<%=sDefaultUrl %>/privacy.asp
...
Easystorecreator reserves the right at any time and from time to time to modify or discontinue, temporarily or permanently, the Service (or any part thereof) with or without notice. You agree that <%=Name %> shall not be liable to you or to any third party for any modification, suspension or discontinuance of the Service.
...
Easystorecreator, and <%=Name %>.com are trademarks of <%=Name %>. all rights reserved.

21. COPYRIGHTS and COPYRIGHT AGENTS

Easystorecreator respects the intellectual property of others, and we ask our users to do the same. If you believe that your work has been copied in a way that constitutes copyright infringement, please provide <%=Name %>'s Copyright Agent the following information:
...
Please report any violations of the TOS to our Customer Care group at <%=sSupport_email %>.
