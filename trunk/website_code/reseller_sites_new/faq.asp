...
onsubmit="window.open('','askwin', 'toolbar=no,scrollbars=yes,resizable=yes,width=400,height=500,left=200,top=100')">
		...
		<input type="hidden" name="title" value="<%= Name %> Help System">
...
<%= Name %> is compatible with over a dozen processors, <a href=integrated-gateways.asp>click here for the full list</a>.<BR><BR>
...
866-324-2764 or email us at: support@<%= host %><BR><BR>
...
required to use the <%= Name %> domain name.<BR><BR>
...
No, <%= Name %> is offered as a service to our users, we do not currently sell the software.
If you would like to resell our products please contact us at resellers@<%= host %><BR><BR>
<B>Do I need to purchase a Secure Site License?</b><BR>
No, you do not need to purchase an SSL Certificate.  <%= Name %> has a secure site license
which is included in all activated accounts.  Your visitors will be automatically
transferred to the secure site https://yourstore.storesecured.com when entering credit card
