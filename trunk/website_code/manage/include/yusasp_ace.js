...
function boxShow_Image()
	...
	window.open(this.ImagePageURL,"NewWindow","scrollbars=YES,width="+this.ImagePageWidth+",height="+this.ImagePageHeight+",left="+popleft+",top="+poptop);
	}
...
function setDisplayMode()
	...
	if(this.displayMode=='RICH')
		...
		if(!this.isFullHTML)
			...
			var sTmp = this.getContent();
			sTmp=sTmp.substr(sTmp.indexOf("</STYLE>")+1);
  		sTmp=sTmp.substr(sTmp.indexOf(">")+1);//">\r\n"
  		if(sTmp.substr(0,2)=="\r\n") sTmp=sTmp.substr(2);
  		var arrTmp = sTmp.split("<META");
		...
		if (arrTmp.length > 1) sContent = arrTmp.join('<br><font color=green>&lt;link href="' + (this.PageStylePath_RelativeTo_EditorPath + this.PageStyle) + '" type=text/css rel=stylesheet&gt;</font>');
		...
		if (arrTmp.length > 1) sContent = arrTmp.join('<br><font color=green>&lt;link href="' + (this.StyleSelectionPath_RelativeTo_EditorPath + this.StyleSelection) + '" type=text/css rel=stylesheet&gt;</font>');
...
function replaceStr(str,replacestr,replacewith)
	{
	var arrTmp = str.split(replacestr);
	if (arrTmp.length > 1) str = arrTmp.join(replacewith);
...
function putContent(sContent)
  ...
  sContent = replaceStr(sContent,"%OBJ_SYS_IMAGES_OBJ%",this.base + "images/images_themes/");
	sContent = replaceStr(sContent,"%OBJ_STORE_IMAGES_OBJ%/",this.base + "images/");
  sContent = replaceStr(sContent,"URL_STRING&","");
  sContent = replaceStr(sContent,"URL_STRING/","");
  sContent = replaceStr(sContent,"URL_STRING","");
  
  sStyle = "<style type='text/css'>.normal {COLOR: "+this.textcolor+";TEXT-DECORATION: none;font-size : "+this.textsize+";font-family :"+this.textface+";}td {COLOR: "+this.textcolor+";TEXT-DECORATION: none;font-size : "+this.textsize+"pt;"
  sStyle = sStyle + "font-family :"+this.textface+";}.big {COLOR: "+this.textcolor+";TEXT-DECORATION: none;font-size : "+this.bigtextsize+"pt;font-family :"+this.textface+";}.small {COLOR: "+this.textcolor+";TEXT-DECORATION: none;font-size : "+this.smalltextsize+"pt;font-family :"+this.textface+";}"
  sStyle = sStyle + ".link {COLOR: "+this.linkcolor+";TEXT-DECORATION: underline;font-size : "+this.linksize+"pt;font-family :"+this.linkface+";}.nav {COLOR: "+this.navcolor+";TEXT-DECORATION: none;font-size : "+this.navsize+"pt;font-family :"+this.navface+";} BODY {BACKGROUND-COLOR: "+this.background
  if (this.backgroundImg == "")
  {
  	sStyle = sStyle + ";BACKGROUND-IMAGE: "+this.backgroundImg
  }
  sStyle = sStyle + "}</style>"

  sContent = sStyle + sContent
  oDoc.write(sContent);
	oDoc.close();
...
function RUN()
	...
	if(this.usePageProperties && !document.getElementById("boxPage"))//boxPage
		{
		document.write("<iframe id=boxPage name=boxPage src='include/box_Page.htm' style='position: absolute; visibility: hidden; z-index: -1;width:1;height:1;'></iframe>");
		}

	if(this.useTable && !document.getElementById("boxTable2"))//boxTable
		{
		document.write("<iframe id=boxTable2 name=boxTable2 src='include/box_Table.htm' style='position: absolute; visibility: hidden; z-index: -1;width:1;height:1;'></iframe>");
		}

	if(this.useSymbol && !document.getElementById("boxSymbol"))//boxSymbol
		{
		document.write("<iframe id=boxSymbol name=boxSymbol src='include/box_Symbol.htm' style='position: absolute; visibility: hidden; z-index: -1;width:1;height:1;'></iframe>");
		}
	}
...
function ACEditor(oName)
	...
	this.ImagePageURL = "";
	this.ImagePageWidth = 460;
	this.ImagePageHeight = 490;
