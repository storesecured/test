...
Dim strTotCust
Dim intTotcust, intNotPaid
dim sqldate,rsdate, rstotcust
intresellerid = session("Resellerid")
currentmonth = month(now)
currentyear = year(now)

'Code for finding the total number of customers

strTotCust = "select count(distinct(fld_customer_id)) as count from tbl_reseller_customer_master where"&_ 
...
'response.write"strTotCust="&strTotCust
'response.end

set rstotcust = conn.execute(strTotCust)
If Not rstotcust.eof then
				intTotcust = trim(rstotcust("count"))				
End If


'****************Code Added on 21st Jan***********************************************************
...
strgetdata = "select count(distinct(fld_customer_id)) as count,sum(fld_amount_pay_to_reseller) as amount "
strgetdata = strgetdata & "from tbl_reseller_customer_master" 
strgetdata = strgetdata & " where fld_customer_id in (select store_id " 
strgetdata = strgetdata & " from store_settings "
strgetdata = strgetdata & " where reseller_id="&intresellerid&" and service_type>0 and trial_version=0) " 
strgetdata = strgetdata & " and month(fld_plan_transaction_date)="&currentmonth&" and year(fld_plan_transaction_date)="&currentyear&""		
...
if not rsselect.eof then
				...
				intNotPaid =  intTotcust - noofcust
        ...
        profitamt = formatnumber(profitamt,2)
  ...
  <%
	...
	%>             
        ...
        <TR>
          ...
          <tr bgColor=#dddddd>
			<td  align="middle">
			Total Customers Referred(Paid/NonPaid)
      ...
      <tr bgColor=#dddddd>
		...
		<td>
			<%bal = csng(profitamt)-csng(paidamt)
			if bal<> "" then 
				test= split(bal,".")
				if test(1) = "" then
					bal=bal&".00"
